...
void  Move::setPieces( std::string s ) // // // // // // // // // // POT DONE 
{
    if (s.size() == REQUIREDLENGTH){ // required length being 4
        for (int i = 0; i < s.size();i++){
            setPiece(i, s.at(i));
        }
    }
    else // if my string is bigger than 4 in length
        throw std::logic_error("invalid string s value");
}
