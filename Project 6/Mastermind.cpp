...
{
    mCurrentScore = Score (m,mWinningMove); // use non default constructor --> constructs answer array
    mBoard.endRound(m, mCurrentScore); // this will add the move m to the move array and the score s to the score array
    mRound++; // we go to the next round
    return( mCurrentScore );
}

// TO DO select the right GAMEOUTCOME for this game
GAMEOUTCOME Mastermind::determineGameOutcome( ) const // // // // POT DONE
{
    GAMEOUTCOME outcome;
    
    if (mCurrentScore.isExactMatch()){ // if score array only contains RIGHT
        outcome = GAMEWON; // won the game
    }
    else if (mRound == TOTALROUNDSALLOWED && !mCurrentScore.isExactMatch()){ // if you havent found the answer in 4 rounds, then you lost
            outcome = GAMELOST;
        }
    else // if you haven't completed 4 rounds, then the game isnt over yet
        outcome = GAMENOTOVER;
